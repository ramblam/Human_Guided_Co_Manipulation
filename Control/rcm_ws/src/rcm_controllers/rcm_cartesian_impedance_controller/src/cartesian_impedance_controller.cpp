...
namespace rcm_controllers
{
  ...
  /*! \brief Calculates a leaky integral: https://en.wikipedia.org/wiki/Leaky_integrator
   *
   * \param[in] update_frequency   Update frequency in Hz
   * \param[in] filter_percentage  Filter percentage
   * \return Filter step
   */
  inline Eigen::Matrix<double, 6, 1> leakyIntegral(const Eigen::Matrix<double, 6, 1> error, const Eigen::Matrix<double, 6, 1> cumulative_error, const double &update_frequency, const double &leak_rate)
  {
    const Eigen::Matrix<double, 6, 1> dy = - leak_rate * cumulative_error + error;
    return cumulative_error + dy*(1.0 / update_frequency);
  }
  ...
  CartesianImpedanceController::CartesianImpedanceController()
  {
    ...
    this->cumulative_error_ = {0., 0., 0., 0., 0., 0.};
  }
  ...
  Eigen::VectorXd CartesianImpedanceController::calculateCommandedTorques()
  {
    ...
    tau_task << this->jacobian_.transpose() * (-this->cartesian_stiffness_ * this->error_ -integral_stiffness_coefficent_ *
      this->cartesian_stiffness_ * leakyIntegral(error_, cumulative_error_, update_frequency_, integral_leak_rate_) - this->cartesian_damping_ * (this->jacobian_ * this->dq_));
